import { CalendarDays, MessageCircle } from "lucide-react";
import { CTAButton } from "@/components/common/CTAButton";
import { SectionContainer } from "@/components/common/SectionContainer";
import type { Dictionary } from "@/i18n/config";

export function ConsultCTASection({ dictionary }: { dictionary: Dictionary }) {
  const { consultCta } = dictionary.services;

  return (
    <SectionContainer className="bg-primary-50">
      <div className="mx-auto max-w-5xl rounded-[24px] bg-primary-50 px-6 py-10 text-center shadow-design-md min-[744px]:px-12 min-[744px]:py-12">
        <h2 className="text-h2 mx-auto max-w-3xl font-bold text-primary-900">
          {consultCta.title}
        </h2>
        <p className="text-body mx-auto mt-3 max-w-2xl text-neutral-700">
          {consultCta.subtitle}
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <CTAButton
            href="/book-appointment"
            icon={<CalendarDays aria-hidden className="size-6" />}
          >
            {dictionary.common.bookAppointment}
          </CTAButton>
          <CTAButton
            href="/contact"
            icon={<MessageCircle aria-hidden className="size-6" />}
            variant="outline"
          >
            {dictionary.common.chatWithUs}
          </CTAButton>
        </div>
      </div>
    </SectionContainer>
  );
}
